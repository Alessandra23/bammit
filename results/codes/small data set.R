...
model1 <- bammitJagsRealData(
  data = irelandSmall, mmu = 90, smu = 10, stheta = 1/100, a = 0.1, b = 0.1,
  nthin = 1, nburnin = 10, Q = 1
)


# Test fixing t = 0, phi = 1 ----------------------------------------------

model2 <- bammitJagsRealDatat0(data = irelandSmall, mmu = 90, smu = 10, stheta = 1/100, a = 0.1, b = 0.1,
                               nthin = 1, nburnin = 10, Q = 1)



# Test fixing g = 0, gamma = 1 ----------------------------------------------
...
g2 <- ggplot(data = gPostCoeffLong, aes(x = value, y = key)) +
  stat_density_ridges(quantile_lines = TRUE,
                      quantiles = c(0.025, 0.5, 0.975),
                      alpha = 0.7,
                      colour = "steelblue",
...
yhat <- predictionBAMMIT(model = model1, data = data)


dfG <- as.data.frame(model1$BUGSoutput$sims.list$g)
names(dfG) <- unique(irelandSmall$Genotype)
dfE <- as.data.frame(model1$BUGSoutput$sims.list$e)
names(dfE) <-  unique(irelandSmall$Environment)
dfT <- as.data.frame(model1$BUGSoutput$sims.list$t)
names(dfT) <-  unique(irelandSmall$Year)

g_names <- colnames(dfG)
e_names <- colnames(dfE)
t_names <- colnames(dfT)
e <- rep(e_names, times = length(g_names))
g <- rep(g_names, each = length(e_names))
t <- rep(t_names, each = length(t_names))
...
limitsInt <- range(labeling::rpretty(-10, 30))
